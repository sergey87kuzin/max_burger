...
class CommonAdminDAL:
    ...
    async def get_full_objects_list(self, page_params: PageParams, custom_filter):
        count_query = custom_filter.filter(select(func.count(self.model.id)))
        count = await self.db_session.execute(count_query)
        query = custom_filter.filter(
            select(self.model)
